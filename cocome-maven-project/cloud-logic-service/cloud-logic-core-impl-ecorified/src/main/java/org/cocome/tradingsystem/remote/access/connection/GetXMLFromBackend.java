...
@Dependent
public class GetXMLFromBackend extends unification.org.cocome.tradingsystem.remote.access.connection.UnifiedGetXMLFromBackend implements IBackendQuery {
	
	private static Logger LOG = Logger.getLogger(GetXMLFromBackend.class);
	...
	public String getXMLFromBackend(String urlString) {
		...
		try {
			...
			note = "url";
			getLOG().info("Opening URL: " + urlString);
			...
			in.close();
			note = "input closed";
			this.setMessage(response.toString());
			return this.getMessage();

		} catch (MalformedURLException ex) {
			getLOG().error("Encountered MalformedURLException: " + ex.toString() +
					"\nCause: " + (ex.getCause() == null ? "No throwable cause" : ex.getCause().toString()) + 
					"\nTrace: ");
			ex.printStackTrace();
			return "MalformedURLException";
		} catch (IOException ex) {
			getLOG().error("Encountered IOException: " + ex.toString() + 
					"\nCause: " + (ex.getCause() == null ? "No throwable cause" : ex.getCause().toString()) +
					"\nTrace: " + " X " + note);
			ex.printStackTrace();
			return "IOExceptions";
	...
	private String getURLToBackend() {
		String url = "http://" + getBackendHost() + ":" + getBackendPort() + getBackendGetDataURL();
		getLOG().info("URL to get data from Backend: " + url);
		return url;
	}
