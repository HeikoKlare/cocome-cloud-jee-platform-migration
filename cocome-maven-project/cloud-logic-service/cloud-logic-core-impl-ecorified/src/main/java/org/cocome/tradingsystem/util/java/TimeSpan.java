 ...
 */
public class TimeSpan extends unification.org.cocome.tradingsystem.util.java.UnifiedTimeSpan implements Comparable<TimeSpan> {
	...
	public long getDuration(final TimeUnit unit) {
		return unit.convert(Math.abs(get__value()), get__unit());
	}
	...
	@Override
	public int hashCode() {
		return __longHashCode(get__value()) ^ get__unit().hashCode();
	}
	...
	public int compareTo(final TimeSpan that) {
		if (this.get__unit().compareTo(that.get__unit()) <= 0) {
			return __compareTo(this.get__unit(), that);
		} else {
			return __compareTo(that.get__unit(), that);
		}
	}

	private int __compareTo(final TimeUnit base, final TimeSpan that) {
		final long thisDuration = base.convert(this.get__value(), this.get__unit());
		final long thatDuration = base.convert(that.get__value(), that.get__unit());
