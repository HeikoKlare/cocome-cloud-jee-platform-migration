...
@RequestScoped
public class CSVHelper extends unification.org.cocome.tradingsystem.remote.access.parsing.UnifiedCSVHelper implements IBackendConversionHelper {
	...
	ICredentialFactory getCredFactory() {
		return credFactory;
	}

	void setCredFactory(ICredentialFactory credFactory) {
		this.credFactory = credFactory;
	}

	Instance<ICredentialFactory> getCredFactoryInstance() {
		return credFactoryInstance;
	}

	void setCredFactoryInstance(Instance<ICredentialFactory> credFactoryInstance) {
		this.credFactoryInstance = credFactoryInstance;
	}

	IUserDataFactory getUserFactory() {
		return userFactory;
	}

	void setUserFactory(IUserDataFactory userFactory) {
		this.userFactory = userFactory;
	}

	IStoreDataFactory getStoreFactory() {
		return storeFactory;
	}

	void setStoreFactory(IStoreDataFactory storeFactory) {
		this.storeFactory = storeFactory;
	}

	IEnterpriseDataFactory getEnterpriseFactory() {
		return enterpriseFactory;
	}

	void setEnterpriseFactory(IEnterpriseDataFactory enterpriseFactory) {
		this.enterpriseFactory = enterpriseFactory;
	}

	private void initCredFactory() {
		if (getCredFactory() == null) {
			setCredFactory(getCredFactoryInstance().get());
		}
	}
	...
	private IStore getStoreFromRow(Row<String> row, int offset) {
		...
		IStore result = getStoreFactory().getNewStore();
	...
	private IProductSupplier getProductSupplierFromEnterpriseRow(Row<String> row) {
		...
		IProductSupplier result = getEnterpriseFactory().getNewProductSupplier();
	...
	private ITradingEnterprise getEnterpriseFromRow(Row<String> row) {
		ITradingEnterprise result = getEnterpriseFactory().getNewTradingEnterprise();
	...
	private IStockItem getStockItemFromRow(Row<String> row) {
		IStockItem result = getStoreFactory().getNewStockItem();
	...
	private IProduct getProductFromRow(Row<String> row) {
		IProduct result = getEnterpriseFactory().getNewProduct();
	...
	private IProductOrder getProductOrderFromRow(Row<String> row) {
		IProductOrder result = getStoreFactory().getNewProductOrder();
	...
	private IOrderEntry getOrderEntryFromRow(Row<String> row) {
		IOrderEntry result = getStoreFactory().getNewOrderEntry();
	...
	public Collection<ICustomer> getCustomers(String input) {
		getLOG().debug("Parsing customer from input: " + input);
		CSVParser parser = new CSVParser();
		parser.parse(input);
		
		int tableRows =  parser.getModel().getRows().size();
		
		getLOG().debug("Parsing customer model: " + parser.getModel().toString() + " with " + tableRows + " rows.");
		...
		if (customers == null) {
			getLOG().warn("No customers found, returning empty list");
	...
	private void extractCustomerRow(HashMap<Long, ICustomer> customers, Row<String> row) {
		getLOG().debug("Parsing row: " + row.toString());
		...
		getLOG().debug("Got from row: " + currentCustomer + "; " + currentUser);
	...
	private ICustomer getCustomerFromRow(Row<String> row) {
		ICustomer result = getUserFactory().getNewCustomer();
	...
	public Collection<IUser> getUsers(String input) {
		...
		getLOG().debug("Parsing user model: " + parser.getModel().toString());
	...
	private void extractUserRow(HashMap<String, IUser> users, Row<String> row) {
		...
		getLOG().debug("Got from row: " + currentUser + "; " + currentRole);
	...
	private ICredential getCredentialFromRow(Row<String> row) {
		...
		ICredential cred = getCredFactory().getCredential(type);
	...
	private IUser getUserFromRow(Row<String> row, int offset) {
		...
		IUser user = getUserFactory().getNewUser();
	...
	public Collection<ITradingEnterprise> getEnterprises(String input) {
		...
		getLOG().debug("Parsing enterprise model: " + parser.getModel().toString());
	...
	private void extractEnterpriseRow(
		...
		getLOG().debug("Got from row: " + currentEnterprise + "; " + currentProductSupplier);
		...
		if (savedEnterprise != null && currentProductSupplier != null) {
			...
			getLOG().debug("Adding product supplier to saved enterprise: " + currentProductSupplier);
			...
			if (currentProductSupplier != null) {
				getLOG().debug("Adding product supplier to current enterprise: " + currentProductSupplier);
				// A ProductSupplier was present, so save it in the enteprise
				newSuppliers.add(currentProductSupplier);
			}
			getLOG().debug("Adding enterprise to map: " + currentEnterprise);
